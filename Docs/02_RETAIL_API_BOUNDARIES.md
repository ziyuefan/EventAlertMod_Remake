...
- 不使用`forceinsecure`，不嘗試繞過污點，也不加入壓制暴雪阻止行動的解決方法。
- 發現污點、被阻止的動作、戰鬥鎖定錯誤時，需記錄到`Docs/15_DEVELOPMENT_ISSUE_LOG.md`。
...
- **阻止光環整合**：對於不需要的或垃圾郵件光環，請利用本機「C_UnitAuras.AddBlockedAura(unit, auraInstanceID)」將過濾委託給 C++ 引擎。只有當插件的執行路徑完全不受污染時才必須呼叫此方法 (`AllowedWhenUntainted`)。
...
- 旧版全局 `GetSpellCooldown`、`GetSpellInfo`、`GetSpellTexture`、

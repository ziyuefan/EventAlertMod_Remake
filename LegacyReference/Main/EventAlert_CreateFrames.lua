...
檔案: LegacyReference\Main\EventAlert_CreateFrames.lua
...
function G:CreateFrames()
	-- Create anchor frames used for mod customization.
		...
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame1", 1) -- (1) self buff
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame2", 1) -- (1) self buff
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame3", 1) -- (1) self buff
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame4", 1) -- (1) self buff
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame5", 2) -- (2) target buff
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame6", 2) -- (2) target buff
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame7", 2) -- (2) target buff
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame8", 2) -- (2) target buff
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame9", 3) -- (3) Skill Cool-Down
		G:CreateFrames_CreateAnchorFrame("EA_Anchor_Frame10", 3) -- (3) Skill Cool-Down
	-- Self Buff/Debuff
	...
	-- Create primary alert frames
		G:CreateFrames_EventsFrame_CreateSpellList(1)
		G:CreateFrames_EventsFrame_RefreshSpellList(1)
		EA_Class_Events_Frame:SetHeight(EA_OptHeight)

	-- Create alternate alert frames
		G:CreateFrames_EventsFrame_CreateSpellList(2)
		G:CreateFrames_EventsFrame_RefreshSpellList(2)
		EA_ClassAlt_Events_Frame:SetHeight(EA_OptHeight)

	-- Create other alert frames. (Mostly trinket procs)
		G:CreateFrames_EventsFrame_CreateSpellList(3)
	...
	-- Create Target's Debuffs alert frames. (Target's Debuffs only now)
		G:CreateFrames_EventsFrame_CreateSpellList(4)
		G:CreateFrames_EventsFrame_RefreshSpellList(4)
		EA_Target_Events_Frame:SetHeight(EA_OptHeight)

	-- Create Spells' Cooldown alert frames.
		G:CreateFrames_EventsFrame_CreateSpellList(5)
		G:CreateFrames_EventsFrame_RefreshSpellList(5)
	...
	G:CreateFrames_CreateMinimapOptionFrame()
end
--------------------------------------------------------------------------------
-- 
--------------------------------------------------------------------------------
function G:CreateFrames_CreateAnchorFrame(AnchorFrameName, typeIndex)
		local eaaf = CreateFrame("FRAME", AnchorFrameName, UIParent)
		eaaf:ClearAllPoints()
		eaaf:SetFrameStrata("MEDIUM")
		-- eaaf:SetFrameStrata("LOW")
		
		Lib_ZYF:SetBackdrop(eaaf,{bgFile = "Interface/Icons/Spell_Nature_Polymorph_Cow"})		

		eaaf.spellName = eaaf:CreateFontString(AnchorFrameName.."_Name","OVERLAY")
		eaaf.spellName:SetFontObject(G.FONT_OBJECT)
		eaaf.spellName:SetPoint("BOTTOM", 0, -15)

		--eaaf.spellTimer = eaaf:CreateFontString(AnchorFrameName.."_Timer","OVERLAY")
		eaaf.spellTimer = eaaf:CreateFontString(AnchorFrameName.."_Timer","OVERLAY")
		eaaf.spellTimer:SetFontObject(G.FONT_OBJECT)
		eaaf.spellTimer:SetPoint("TOP", 0, 15)

		eaaf:SetMovable(true)
		eaaf:EnableMouse(true)
		if (typeIndex == 1) then
			eaaf:SetScript("OnMouseDown",   G.Icon_Options_Frame_Anchor_OnMouseDown)
			eaaf:SetScript("OnMouseUp",     G.Icon_Options_Frame_Anchor_OnMouseUp)
		elseif (typeIndex == 2) then
			eaaf:SetScript("OnMouseDown",   G.Icon_Options_Frame_Anchor_OnMouseDown2)
			eaaf:SetScript("OnMouseUp",     G.Icon_Options_Frame_Anchor_OnMouseUp2)
		elseif (typeIndex == 3) then
			eaaf:SetScript("OnMouseDown",   G.Icon_Options_Frame_Anchor_OnMouseDown3)
			eaaf:SetScript("OnMouseUp",     G.Icon_Options_Frame_Anchor_OnMouseUp3)
		end
		eaaf:Hide()
end
--------------------------------------------------------------------------------
-- 
--------------------------------------------------------------------------------
function G:CreateFrames_CreateSpellFrame(index, typeIndex)
	...
	eaf.spellName:SetFontObject(G.FONT_OBJECT)
	eaf.spellName:SetPoint("TOP", eaf, "BOTTOM", 0, -5)	

	eaf.spellTimer:SetFontObject(G.FONT_OBJECT)
	eaf.spellTimer:SetPoint("TOP", 0, EA_Config.TimerFontSize)

	eaf.spellStack:SetFontObject(G.FONT_OBJECT)
	...
	if typeIndex == 1 then
		if G.SPELLINFO_SELF[spellId] == nil then 
			G.SPELLINFO_SELF[spellId] = {name,  icon, count, duration, expirationTime, unitCaster, isDebuff} 
		end
		
		G.SPELLINFO_SELF[spellId].name = name
		...
		G.SPELLINFO_SELF[spellId].icon = icon
		
	elseif typeIndex == 2 then
		if G.SPELLINFO_TARGET[spellId] == nil then 
			G.SPELLINFO_TARGET[spellId] = {name,  icon, count, duration, expirationTime, unitCaster, isDebuff} 
		end
		
		G.SPELLINFO_TARGET[spellId].name = name		
		G.SPELLINFO_TARGET[spellId].icon = icon
		
	elseif typeIndex == 3 then				
		
		if G.SPELLINFO_SCD[spellId] == nil then 
			G.SPELLINFO_SCD[spellId] = {name, icon}			
		end
		
		
		G.SPELLINFO_SCD[spellId].name = name				
		G.SPELLINFO_SCD[spellId].icon = icon
		
		for k, v in pairs(EA_ScdItems[G.playerClass][spellId]) do			
			
			if G.SPELLINFO_SCD[spellId] then				
				G.SPELLINFO_SCD[spellId][k] = v
			end
		end 
...
--------------------------------------------------------------------------------
function G:CreateFrames_SpecialFrames_Show(index)
	...
	if (eaf ~= nil) then 
		...
		elseif (iPowerType == EA_SpecPower.Runes.powerId) then			
			...
			G:UpdateSinglePower(iPowerType)
		end		
		return
	end
...
-- 
--------------------------------------------------------------------------------
function G:CreateFrames_SpecialFrames_Hide(index)
...
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_CreateScrollFrame(ParentFrameObj, ScrollFrameHeight, xOffset, yOffset)
	...
	local scc = CreateFrame("Frame", framename.."_SpellListFrame", panel3)
		...
		Lib_ZYF:SetBackdrop(panel3,{bgFile="Interface\\DialogFrame\\UI-DialogBox-Background-Dark", edgeFile="", tile = false, tileSize = 0, edgeSize = 0.01, insets = { left = 0, right = 0, top = 0, bottom = 0 }})
...
-- 
--------------------------------------------------------------------------------
function G:CreateFrames_CreateSpellListIcon(SpellID, FrameNamePrefix, ParentFrameObj, LocOffsetX, LocOffsetY, IconPath)
...
-- 
--------------------------------------------------------------------------------
function G.ChkboxGetChecked(self)
	...
	if (iFrameIndex == 1) then
		EA_Items[G.playerClass][iSpellID].enable = fValue
	elseif (iFrameIndex == 2) then
		EA_AltItems[G.playerClass][iSpellID].enable = fValue
	elseif (iFrameIndex == 3) then
		EA_Items[EA_CLASS_OTHER][iSpellID].enable = fValue
	elseif (iFrameIndex == 4) then
		EA_TarItems[G.playerClass][iSpellID].enable = fValue
	elseif (iFrameIndex == 5) then
		EA_ScdItems[G.playerClass][iSpellID].enable = fValue
	elseif (iFrameIndex == 6) then
		EA_GrpItems[G.playerClass][iSpellID].enable = fValue
	end
end                                                                             
--------------------------------------------------------------------------------
-- 
--------------------------------------------------------------------------------
function G.ChkboxGameToolTip(self)
...
-- 
--------------------------------------------------------------------------------
function G:CreateFrames_CreateSpellListChkbox(SpellID, FrameNamePrefix, ParentFrameObj, LocOffsetX, LocOffsetY, SpellName, FrameIndex, EditboxObj)
	...
	if FrameIndex == 1 then
		fValue = EA_Items[G.playerClass][SpellID].enable
	elseif FrameIndex == 2 then
		fValue = EA_AltItems[G.playerClass][SpellID].enable
	elseif FrameIndex == 3 then
		fValue = EA_Items[EA_CLASS_OTHER][SpellID].enable
	elseif FrameIndex == 4 then
		fValue = EA_TarItems[G.playerClass][SpellID].enable
	elseif FrameIndex == 5 then
		fValue = EA_ScdItems[G.playerClass][SpellID].enable
	elseif FrameIndex == 6 then
		fValue = EA_GrpItems[G.playerClass][SpellID].enable
		iTooltipSpellID = EA_GrpItems[G.playerClass][SpellID].Spells[1].SpellIconID
	end
	...
	SpellChkbox:SetScript("OnClick", G.ChkboxGetChecked)
	SpellChkbox:SetScript("OnEnter", G.ChkboxGameToolTip)
	SpellChkbox:Show()
end
--------------------------------------------------------------------------------
-- 
...
--      SpellEditBox:SetFontObject(G.FONT_OBJECT)
...
function G.CreateFrames_CfgBtn_SaveSpellCondition(self)
	...
	local Chk_Aura 	= table.create(4)
	local SC_Aura 	= table.create(4)
	SC_Aura = { nil, nil, nil, nil }
	Chk_Aura = { false, false, false, false } 	
	...
	SC_OrderWtd = tonumber(SC_OrderWtd)
	...
	for i = 1, 4 do 
		Chk_Aura[i], SC_Aura[i] = CreateFrames_CfgBtnFun_GetChkText(_G["EA_SpellCondition_Frame_AuraValue"..i], _G["EA_SpellCondition_Frame_AuraValue"..i.."_EditBox"])
		if (not Chk_Aura[i]) or  (SC_Aura[i] == nil)  then 
			Chk_Aura[i], SC_Aura[i] = false, nil 
		end
	end
	-- Chk_Aura1, SC_Aura1 = CreateFrames_CfgBtnFun_GetChkText(EA_SpellCondition_Frame_AuraValue1, EA_SpellCondition_Frame_AuraValue1_EditBox)
	-- if (not Chk_Aura1) or  (SC_Aura1 == nil)  then Chk_Aura1, SC_Aura1 = false,nil end
	-- Chk_Aura2, SC_Aura2 = CreateFrames_CfgBtnFun_GetChkText(EA_SpellCondition_Frame_AuraValue2, EA_SpellCondition_Frame_AuraValue2_EditBox)
	-- if (not Chk_Aura2) or  (SC_Aura2 == nil)  then Chk_Aura2, SC_Aura2 = false,nil end
	-- Chk_Aura3, SC_Aura3 = CreateFrames_CfgBtnFun_GetChkText(EA_SpellCondition_Frame_AuraValue3, EA_SpellCondition_Frame_AuraValue3_EditBox)
	-- if (not Chk_Aura3) or  (SC_Aura3 == nil)  then Chk_Aura3, SC_Aura3 = false,nil end
	-- Chk_Aura4, SC_Aura4 = CreateFrames_CfgBtnFun_GetChkText(EA_SpellCondition_Frame_AuraValue4, EA_SpellCondition_Frame_AuraValue4_EditBox)	
	-- if (not Chk_Aura4) or  (SC_Aura4 == nil)  then Chk_Aura4, SC_Aura4 = false,nil end
	...
	local function CreateFrames_CfgBtnFun_SaveToItem(EAItem)
		...
		EAItem.auravalue = EAItem.auravalue or {}
		for i = 1, 4 do 
			EAItem.auravalue[i] = EAItem.auravalue[i] or {}			
			EAItem.auravalue[i].enabled = Chk_Aura[i]
			EAItem.auravalue[i].label = SC_Aura[i]
			
		end
		-- EAItem.auravalue1 = Chk_Aura1
		-- EAItem.auravaluelabel1 = SC_Aura1
		
		-- EAItem.auravalue2 = Chk_Aura2
		-- EAItem.auravaluelabel2 = SC_Aura2
		
		-- EAItem.auravalue3 = Chk_Aura3
		-- EAItem.auravaluelabel3 = SC_Aura3
		
		-- EAItem.auravalue4 = Chk_Aura4
		-- EAItem.auravaluelabel4 = SC_Aura4
		
		
	end
	if (FrameIndex == 1) then
		CreateFrames_CfgBtnFun_SaveToItem(EA_Items[G.playerClass][SpellID])
	-- elseif (FrameIndex == 2) then
	-- 	CreateFrames_CfgBtnFun_SaveToItem(EA_AltItems[G.playerClass][SpellID])
	elseif (FrameIndex == 3) then
		CreateFrames_CfgBtnFun_SaveToItem(EA_Items[EA_CLASS_OTHER][SpellID])
	elseif (FrameIndex == 4) then
		CreateFrames_CfgBtnFun_SaveToItem(EA_TarItems[G.playerClass][SpellID])
	elseif (FrameIndex == 5) then
		CreateFrames_CfgBtnFun_SaveToItem(EA_ScdItems[G.playerClass][SpellID])
	end
...
-- 
--------------------------------------------------------------------------------
function G:EACFFun_EventsFrame_CheckSpellID(spellID, ReCheckSpell)
	local spellInfo = GetSpellInfo(spellID)
	
	local EA_name =  (type(spellInfo) == "table") and spellInfo.name or select(1, spellInfo)
...
function G.CreateFrames_CfgBtn_LoadSpellCondition(self)
	-- Get saved condition of spell
	local FrameIndex = self.FrameIndex
	local SpellID = self.SpellID
	local ReCheckSpell = false
	if (FrameIndex == 1) then ReCheckSpell = true end
	local SpellName,  SpellIconPath = G:EACFFun_EventsFrame_CheckSpellID(SpellID, ReCheckSpell)	
	...
	SC_Aura  = table.create(4)
	Chk_Aura = table.create(4)
	SC_Aura = {nil, nil, nil, nil}
	Chk_Aura = {[1]=false, [2]=false, [3]=false, [4]=false}  
	...
	local function CreateFrames_CfgBtnFun_GetFromSave(EAItem) 		
		...
		local av = EAItem.auravalue
		for i = 1, 4 do					
			Chk_Aura[i] = av and av[i] and av[i].enabled
			SC_Aura[i]  = av and av[i] and av[i].label
		end
		-- Chk_Aura1 = EAItem.auravalue1
		-- SC_Aura1  = EAItem.auravaluelabel1
		-- Chk_Aura2 = EAItem.auravalue12
		-- SC_Aura2  = EAItem.auravaluelabel2
		-- Chk_Aura3 = EAItem.auravalue3
		-- SC_Aura3  = EAItem.auravaluelabel3
		-- Chk_Aura4 = EAItem.auravalue4
		-- SC_Aura4  = EAItem.auravaluelabel4
	end
	
	--
	if (FrameIndex == 1) then
		CreateFrames_CfgBtnFun_GetFromSave(EA_Items[G.playerClass][SpellID])
	-- elseif (FrameIndex == 2) then
	-- 	CreateFrames_CfgBtnFun_GetFromSave(EA_AltItems[G.playerClass][SpellID])
	elseif (FrameIndex == 3) then
		CreateFrames_CfgBtnFun_GetFromSave(EA_Items[EA_CLASS_OTHER][SpellID])
	elseif (FrameIndex == 4) then
		CreateFrames_CfgBtnFun_GetFromSave(EA_TarItems[G.playerClass][SpellID])
	elseif (FrameIndex == 5) then
		CreateFrames_CfgBtnFun_GetFromSave(EA_ScdItems[G.playerClass][SpellID])
	end
	...
	if FrameIndex >= 1 and FrameIndex <= 4 then
		-- CreateFrames_CfgBtnFun_SetChkText(EA_SpellCondition_Frame_AuraValue1, EA_SpellCondition_Frame_AuraValue1_EditBox, Chk_Aura1, SC_Aura1)
		-- CreateFrames_CfgBtnFun_SetChkText(EA_SpellCondition_Frame_AuraValue2, EA_SpellCondition_Frame_AuraValue2_EditBox, Chk_Aura2, SC_Aura2)
		-- CreateFrames_CfgBtnFun_SetChkText(EA_SpellCondition_Frame_AuraValue3, EA_SpellCondition_Frame_AuraValue3_EditBox, Chk_Aura3, SC_Aura3)
		-- CreateFrames_CfgBtnFun_SetChkText(EA_SpellCondition_Frame_AuraValue4, EA_SpellCondition_Frame_AuraValue4_EditBox, Chk_Aura4, SC_Aura4)
		
		for i = 1, 4 do
			_G["EA_SpellCondition_Frame_AuraValue" .. i]:Show()
			_G["EA_SpellCondition_Frame_AuraValue" .. i .. "_EditBox"]:Show()
			CreateFrames_CfgBtnFun_SetChkText(_G["EA_SpellCondition_Frame_AuraValue"..i], _G["EA_SpellCondition_Frame_AuraValue"..i.."_EditBox"], Chk_Aura[i], SC_Aura[i])
		end	
	else	
		for i = 1, 4 do
			_G["EA_SpellCondition_Frame_AuraValue" .. i]:Hide()
			_G["EA_SpellCondition_Frame_AuraValue" .. i .. "_EditBox"]:Hide()
		end		
	end
	...
	EA_SpellCondition_Frame_Save:SetScript("OnClick", G.CreateFrames_CfgBtn_SaveSpellCondition)
...
--
-------------------------------------------------------------------------
function G:CreateFrames_CreateSpellListCfgBtn(SpellID, FrameNamePrefix, ParentFrameObj, LocOffsetX, LocOffsetY, FrameIndex)
	SpellID = tonumber(SpellID)
	...
	SpellCfgBtn:SetNormalTexture(G.Image_Path.."UI-Panel-CfgButton-Down")
	SpellCfgBtn:SetHighlightTexture(G.Image_Path.."UI-Panel-CfgButton-Highlight", "BLEND")
	if (FrameIndex <= 4) then
		SpellCfgBtn.FrameIndex = FrameIndex
		SpellCfgBtn.SpellID = SpellID
		SpellCfgBtn:SetScript("OnClick", G.CreateFrames_CfgBtn_LoadSpellCondition)
	...
	elseif (FrameIndex == 6) then
		...
		SpellCfgBtn:SetScript("OnClick", G.CreateFrames_CfgBtn_LoadGroupEvent)
		-- SpellCfgBtn:SetScript("OnClick", function()
...
--
-------------------------------------------------------------------------
function G.CreateFrames_CfgBtn_GroupFrame_OnMouseDown(self)
	self:StartMoving()
end
-------------------------------------------------------------------------
--
-------------------------------------------------------------------------
function G.CreateFrames_CfgBtn_GroupFrame_OnMouseUp(self)
	local iGroupID = self.GroupID
	self:StopMovingOrSizing()
	local EA_point, _, EA_relativePoint, EA_xOfs, EA_yOfs = self:GetPoint()
	-- EA_GrpItems[G.playerClass][iGroupID].IconPoint = EA_point
	-- EA_GrpItems[G.playerClass][iGroupID].IconRelatePoint = EA_relativePoint
	-- EA_GrpItems[G.playerClass][iGroupID].LocX = EA_xOfs
	-- EA_GrpItems[G.playerClass][iGroupID].LocY = EA_yOfs
...
--
-------------------------------------------------------------------------
function G.CreateFrames_CfgBtn_ShowGroupFramePos(self)
	local iGroupID = self.GroupID
	local FrameNamePrefix = "EAGrpAnchorFrame_"
	local aGroupChecks = EA_GrpItems[G.playerClass][iGroupID]
	...
	if (eaf == nil) then
		...
		eaf:SetScript("OnMouseDown", G.CreateFrames_CfgBtn_GroupFrame_OnMouseDown)
		eaf:SetScript("OnMouseUp", G.CreateFrames_CfgBtn_GroupFrame_OnMouseUp)
		eaf:ClearAllPoints()
		eaf:SetFrameStrata("MEDIUM")		
...
--
-------------------------------------------------------------------------
function G:CreateFrames_CreateSpellListCfgBtn2(SpellID, FrameNamePrefix, ParentFrameObj, LocOffsetX, LocOffsetY, FrameIndex)
	SpellID = tonumber(SpellID)
	...
	SpellCfgBtn.GroupID = SpellID
	SpellCfgBtn:SetScript("OnClick",G.CreateFrames_CfgBtn_ShowGroupFramePos)
	SpellCfgBtn:Show()
end
...
--      SpellFontStr = ParentFrameObj:CreateFontString(FrameNamePrefix..SpellID, "ARTWORK", G.FONT_OBJECT:GetName())
...
--------------------------------------------------------------------------------
function G.CreateFrames_CfgBtn_LoadGroupEvent(self)
	...
	G:EAFun_GroupEvent_LoadGroupEventToFrame(GroupID)
end
--------------------------------------------------------------------------------
-- CreateSpellList, ClearSpellList, RefreshSpellList
--------------------------------------------------------------------------------
function G:EACFFun_EventsFrame_CreateSpellList(EAItems, typeIndex)
	for index, value in G:pairsByKeys(EAItems) do
		G:CreateFrames_CreateSpellFrame(index, typeIndex)
	end
end
--------------------------------------------------------------------------------
--
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_CreateSpellList(FrameIndex)
	if (FrameIndex == 1) then
		G:EACFFun_EventsFrame_CreateSpellList(EA_Items[G.playerClass], 1)
	elseif (FrameIndex == 2) then
		G:EACFFun_EventsFrame_CreateSpellList(EA_AltItems[G.playerClass], 1)
	elseif (FrameIndex == 3) then
		G:EACFFun_EventsFrame_CreateSpellList(EA_Items[EA_CLASS_OTHER], 1)
		G:EACFFun_EventsFrame_CreateSpellList(EA_Items[EA_CLASS_OTHER], 2)
	elseif (FrameIndex == 4) then
		G:EACFFun_EventsFrame_CreateSpellList(EA_TarItems[G.playerClass], 2)
	elseif (FrameIndex == 5) then
		G:EACFFun_EventsFrame_CreateSpellList(EA_ScdItems[G.playerClass], 3)
	elseif (FrameIndex == 6) then
		for iGrpIndex, aGrpChecks in ipairs (EA_GrpItems[G.playerClass]) do
			G:CreateFrames_CreateGroupCheckFrame(iGrpIndex)
		end
	end
end
--------------------------------------------------------------------------------
--
--------------------------------------------------------------------------------
function G:EACFFun_EventsFrame_ClearSpellList(EAItems, FrameNamePrefix)
	local f1, f2, f3, f4
	for index,value in G:pairsByKeys(EAItems) do
...
--
--------------------------------------------------------------------------------
function G:EACFFun_EventsFrame_ClearSpellFrame(EAItems, FrameNamePrefix)
	local eaf = nil
...
--
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_ClearSpellList(FrameIndex)
	if (FrameIndex == 1) then
		G:EACFFun_EventsFrame_ClearSpellList(EA_Items[G.playerClass], "EA_ClassFrame")
	elseif (FrameIndex == 2) then
		G:EACFFun_EventsFrame_ClearSpellList(EA_AltItems[G.playerClass], "EA_ClassAltFrame")
	elseif (FrameIndex == 3) then
		G:EACFFun_EventsFrame_ClearSpellList(EA_Items[EA_CLASS_OTHER], "EA_OtherFrame")
	elseif (FrameIndex == 4) then
		G:EACFFun_EventsFrame_ClearSpellList(EA_TarItems[G.playerClass], "EA_TargetFrame")
	elseif (FrameIndex == 5) then
		G:EACFFun_EventsFrame_ClearSpellList(EA_ScdItems[G.playerClass], "EA_SCDFrame")
	elseif (FrameIndex == 6) then
		G:EACFFun_EventsFrame_ClearSpellList(EA_GrpItems[G.playerClass], "EA_GroupFrame")
		G:EACFFun_EventsFrame_ClearSpellFrame(EA_GrpItems[G.playerClass], "EAGrpFrame_")
	end
end
--------------------------------------------------------------------------------
--
--------------------------------------------------------------------------------
function G:EACFFun_EventsFrame_RefreshSpellList(FrameIndex, EAItems, FrameNamePrefix, ParentFrameObj, LocOffsetX, LocOffsetY, EditboxObj)
	...
	for index, value in G:pairsByKeys(EAItems) do
		...
		local EA_name, EA_icon = G:EACFFun_EventsFrame_CheckSpellID(index, ReCheckSpell)
		G:CreateFrames_CreateSpellListIcon(index, FrameNamePrefix.."_Icon_", ParentFrameObj, LocOffsetX, LocOffsetY, EA_icon)
		G:CreateFrames_CreateSpellListChkbox(index, FrameNamePrefix.."_ChkBtn_", ParentFrameObj, LocOffsetX, LocOffsetY, EA_name, FrameIndex, EditboxObj)
		if (CanCfg) then G:CreateFrames_CreateSpellListCfgBtn(index, FrameNamePrefix.."_CfgBtn_", ParentFrameObj, LocOffsetX, LocOffsetY, FrameIndex) end
...
--
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_RefreshSpellList(FrameIndex)
	if (FrameIndex == 1) then
		G:EACFFun_EventsFrame_RefreshSpellList(FrameIndex, EA_Items[G.playerClass], "EA_ClassFrame", EA_Class_Events_Frame_SpellScrollFrameList, 0, 0, EA_Class_Events_Frame_SpellEditBox)
	elseif (FrameIndex == 2) then
		G:EACFFun_EventsFrame_RefreshSpellList(FrameIndex, EA_AltItems[G.playerClass], "EA_ClassAltFrame", EA_ClassAlt_Events_Frame_SpellScrollFrameList, 0, 0, EA_ClassAlt_Events_Frame_SpellEditBox)
	elseif (FrameIndex == 3) then
		G:EACFFun_EventsFrame_RefreshSpellList(FrameIndex, EA_Items[EA_CLASS_OTHER], "EA_OtherFrame", EA_Other_Events_Frame_SpellScrollFrameList, 0, 0, EA_Other_Events_Frame_SpellEditBox)
	elseif (FrameIndex == 4) then
		G:EACFFun_EventsFrame_RefreshSpellList(FrameIndex, EA_TarItems[G.playerClass], "EA_TargetFrame", EA_Target_Events_Frame_SpellScrollFrameList, 0, 0, EA_Target_Events_Frame_SpellEditBox)
	elseif (FrameIndex == 5) then
		G:EACFFun_EventsFrame_RefreshSpellList(FrameIndex, EA_ScdItems[G.playerClass], "EA_SCDFrame", EA_SCD_Events_Frame_SpellScrollFrameList, 0, 0, EA_SCD_Events_Frame_SpellEditBox)
	elseif (FrameIndex == 6) then
		local LocOffsetY = 0
		local EA_name, EA_rank, EA_icon = "", "", ""
		for iGrpIndex, aGrpChecks in ipairs (EA_GrpItems[G.playerClass]) do
			EA_name, EA_icon = G:EACFFun_EventsFrame_CheckSpellID(aGrpChecks.Spells[1].SpellIconID, false)
			...
			G:CreateFrames_CreateSpellListIcon(iGrpIndex, "EA_GroupFrame_Icon_", EA_Group_Events_Frame_SpellScrollFrameList, 0, LocOffsetY, EA_icon)
			G:CreateFrames_CreateSpellListChkbox(iGrpIndex, "EA_GroupFrame_ChkBtn_", EA_Group_Events_Frame_SpellScrollFrameList, 0, LocOffsetY, EA_name, FrameIndex, EA_Group_Events_Frame_SpellEditBox)
			G:CreateFrames_CreateSpellListCfgBtn(iGrpIndex, "EA_GroupFrame_CfgBtn_", EA_Group_Events_Frame_SpellScrollFrameList, 0, LocOffsetY, FrameIndex)
			G:CreateFrames_CreateSpellListCfgBtn2(iGrpIndex, "EA_GroupFrame_CfgBtn2_", EA_Group_Events_Frame_SpellScrollFrameList, -25, LocOffsetY, FrameIndex)
			LocOffsetY = LocOffsetY - 25
		end
	end
end
...
function G:CreateFrames_CreateGroupCheckFrame(iGroupIndex)
	
	local FrameNamePrefix = "EAGrpFrame_"
	local aGroupChecks = EA_GrpItems[G.playerClass][iGroupIndex]
	local FrameName = FrameNamePrefix..iGroupIndex
	local eaf = _G[FrameName]
	if (eaf == nil) then
		eaf = CreateFrame("Frame", FrameName, UIParent)
		eaf.spellName  = eaf:CreateFontString(table.concat({FrameName, "_Name" }), "OVERLAY")
		eaf.spellTimer = eaf:CreateFontString(table.concat({FrameName, "_Timer"}), "OVERLAY")
		eaf.spellStack = eaf:CreateFontString(table.concat({FrameName, "_Stack"}), "OVERLAY")
	end
	-- eaf.noCooldownCount = true
	eaf:ClearAllPoints()
	eaf:SetFrameStrata("MEDIUM")	
	...
	eaf.GC = aGroupChecks
        ...
        local aGroupFrameEvents = wipe(aGroupFrameEvents)        
		local aGroupFrameIndexs = wipe(aGroupFrameIndexs)        
		-- local aGroupFrameEvents = {}
		-- local aGroupFrameIndexs = {}
        local sEventType, iEventSeq, sname, iconpath = "", 0, "", ""
	
	for iInd_i, aValue_i in ipairs(aGroupChecks.Spells) do
		local spellInfo = GetSpellInfo(aValue_i.SpellIconID)
	...
	for sEventType, iEventSeq in pairs(aGroupFrameEvents) do
		...
		if G.GC_IndexOfGroupFrame[sEventType] == nil then G.GC_IndexOfGroupFrame[sEventType] = {} end
		if G.GC_IndexOfGroupFrame[sEventType][iGroupIndex] == nil then G.GC_IndexOfGroupFrame[sEventType][iGroupIndex] = {} end
		G.GC_IndexOfGroupFrame[sEventType][iGroupIndex] = aGroupFrameIndexs[sEventType][iGroupIndex]
	end
	...
	eaf:SetScript("OnEvent", G.GroupFrameCheck_OnEvent)
...
--------------------------------------------------------------------------------
function G:EACFFun_EventsFrame_SelAll(EAItems, FrameName, Status)
	local tonumber = tonumber
	for index, value in G:pairsByKeys(EAItems) do
...
--
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_SelAll(FrameIndex, Status)
	if (FrameIndex == 1) then
		G:EACFFun_EventsFrame_SelAll(EA_Items[G.playerClass], "EA_ClassFrame_ChkBtn_", Status)
	elseif (FrameIndex == 2) then
		G:EACFFun_EventsFrame_SelAll(EA_AltItems[G.playerClass], "EA_ClassAltFrame_ChkBtn_", Status)
	elseif (FrameIndex == 3) then
		G:EACFFun_EventsFrame_SelAll(EA_Items[EA_CLASS_OTHER], "EA_OtherFrame_ChkBtn_", Status)
	elseif (FrameIndex == 4) then
		G:EACFFun_EventsFrame_SelAll(EA_TarItems[G.playerClass], "EA_TargetFrame_ChkBtn_", Status)
	elseif (FrameIndex == 5) then
		G:EACFFun_EventsFrame_SelAll(EA_ScdItems[G.playerClass], "EA_SCDFrame_ChkBtn_", Status)
	elseif (FrameIndex == 6) then
		G:EACFFun_EventsFrame_SelAll(EA_GrpItems[G.playerClass], "EA_GroupFrame_ChkBtn_", Status)
	end
end
--------------------------------------------------------------------------------
--
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_LoadDefault(FrameIndex)
	G:CreateFrames_EventsFrame_ClearSpellList(FrameIndex)
	G:LoadClassSpellArray(FrameIndex)
	G:CreateFrames_EventsFrame_CreateSpellList(FrameIndex)
	G:CreateFrames_EventsFrame_RefreshSpellList(FrameIndex)
end
--------------------------------------------------------------------------------
--
--------------------------------------------------------------------------------
function G:EACFFun_GetSpellButton_ByFrame(FrameIndex)
	if (FrameIndex == 1) then
...
-- 新增法術
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_AddSpell(FrameIndex)
	if (FrameIndex <= 5) then		
	
		local SpellButton = G:EACFFun_GetSpellButton_ByFrame(FrameIndex)
		...
		if spellID ~= nil and spellID ~= "" then
			...
			if (sname ~= nil) then
				...
				if (FrameIndex==1 and EA_Items[G.playerClass][spellID] == nil)		then EA_Items[G.playerClass][spellID] 		= {enable=true,name=sname,self=true} end
				if (FrameIndex==2 and EA_AltItems[G.playerClass][spellID] == nil) 	then EA_AltItems[G.playerClass][spellID] 	= {enable=true,name=sname} end
				if (FrameIndex==3 and EA_Items[EA_CLASS_OTHER][spellID] == nil) 	then EA_Items[EA_CLASS_OTHER][spellID] 		= {enable=true,name=sname} end
				if (FrameIndex==4 and EA_TarItems[G.playerClass][spellID] == nil)	then EA_TarItems[G.playerClass][spellID]	= {enable=true,name=sname,self=true} end
				if (FrameIndex==5 and EA_ScdItems[G.playerClass][spellID] == nil)	then EA_ScdItems[G.playerClass][spellID] 	= {enable=true,name=sname,duration=tonumber(duration)} end
				
				local typeIndex = 1		
				if (FrameIndex == 4) then typeIndex = 2 end
				if (FrameIndex == 5) then typeIndex = 3 end
				if (FrameIndex == 3) then 
					G:CreateFrames_CreateSpellFrame(spellID, 1)
					G:CreateFrames_CreateSpellFrame(spellID, 2)
				else
					G:CreateFrames_CreateSpellFrame(spellID, typeIndex)
				end
				G:CreateFrames_EventsFrame_RefreshSpellList(FrameIndex)
			end
		end
	elseif (FrameIndex == 6) then
		...
		-- CreateFrames_CfgBtn_LoadGroupEvent(#EA_GrpItems[G.playerClass] + 1)
		EA_Group_Events_Frame_AddSpell.GroupID = #EA_GrpItems[G.playerClass] + 1
		G.CreateFrames_CfgBtn_LoadGroupEvent(EA_Group_Events_Frame_AddSpell)
	end
end
--------------------------------------------------------------------------------
--
...
function G:EACFFun_EventsFrame_DelSpell(FrameIndex, spellID, EAItems, FrameName)
	local spellID = tonumber(spellID)
	local TempPlayerClass = wipe(TempPlayerClass)
	...
	if (IsCurrSpell) then
		G:CreateFrames_EventsFrame_ClearSpellList(FrameIndex)
		if (FrameIndex == 1) then
			EA_Items[G.playerClass] = TempPlayerClass
		elseif (FrameIndex == 2) then
			EA_AltItems[G.playerClass] = TempPlayerClass
		elseif (FrameIndex == 3) then
			EA_Items[EA_CLASS_OTHER] = TempPlayerClass
		elseif (FrameIndex == 4) then
			EA_TarItems[G.playerClass] = TempPlayerClass
		elseif (FrameIndex == 5) then
			EA_ScdItems[G.playerClass] = TempPlayerClass
		end
...
--
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_DelSpell(FrameIndex)
	local SpellButton = G:EACFFun_GetSpellButton_ByFrame(FrameIndex)
	SpellButton:ClearFocus()
	local spellID = SpellButton:GetText()
	if spellID ~= nil and spellID ~= "" then
		if (FrameIndex == 1) then
			G:EACFFun_EventsFrame_DelSpell(FrameIndex, spellID, EA_Items[G.playerClass], "EAFrame_")
		elseif (FrameIndex == 2) then
			G:EACFFun_EventsFrame_DelSpell(FrameIndex, spellID, EA_AltItems[G.playerClass], "EAFrame_")
		elseif (FrameIndex == 3) then
			G:EACFFun_EventsFrame_DelSpell(FrameIndex, spellID, EA_Items[EA_CLASS_OTHER], "EAFrame_")
		elseif (FrameIndex == 4) then
			G:EACFFun_EventsFrame_DelSpell(FrameIndex, spellID, EA_TarItems[G.playerClass], "EATarFrame_")
		elseif (FrameIndex == 5) then
			G:EACFFun_EventsFrame_DelSpell(FrameIndex, spellID, EA_ScdItems[G.playerClass], "EAScdFrame_")
		elseif (FrameIndex == 6) then
			-- delete all frames
			G:CreateFrames_EventsFrame_ClearSpellList(6)
			-- delete from GrpItems
			table.remove(EA_GrpItems[G.playerClass], spellID)
			-- wipe G.GC_IndexOfGroupFrame
...
--
--------------------------------------------------------------------------------
function G:EACFFun_EventsFrame_RemoveAll(FrameIndex, EAItems, FrameName)
...
--
--------------------------------------------------------------------------------
function G:CreateFrames_EventsFrame_RemoveAllSpells(FrameIndex)
	if (FrameIndex == 1) then
		G:EACFFun_EventsFrame_RemoveAll(FrameIndex, EA_Items[G.playerClass], "EAFrame_")
	elseif (FrameIndex == 2) then
		G:EACFFun_EventsFrame_RemoveAll(FrameIndex, EA_AltItems[G.playerClass], "EAFrame_")
	elseif (FrameIndex == 3) then
		G:EACFFun_EventsFrame_RemoveAll(FrameIndex, EA_Items[EA_CLASS_OTHER], "EAFrame_")
	elseif (FrameIndex == 4) then
		G:EACFFun_EventsFrame_RemoveAll(FrameIndex, EA_TarItems[G.playerClass], "EATarFrame_")
	elseif (FrameIndex == 5) then
		G:EACFFun_EventsFrame_RemoveAll(FrameIndex, EA_ScdItems[G.playerClass], "EAScdFrame_")
	end
end
--------------------------------------------------------------------------------
--
--------------------------------------------------------------------------------
function G:CreateFrames_CreateMinimapOptionFrame()
	...
	-- if eaf.Backdrop == nil then
		...
		-- eaf.Backdrop:SetAllPoints()
		-- eaf.Backdrop.backdropInfo ={bgFile = "Interface/Icons/Trade_Engineering"}
		-- eaf.Backdrop:ApplyBackdrop()
	-- end
	...
	eaf:SetScript("OnEnter", function()	
								...
								t = t.."\124r"
								t = t.." - Show tooltip when mouse hover all frame \n"
								
								t = t.."\124cff00FF00"
								t = t.."/eam NewLineByIconCount (2~n)"
								t = t.."\124r"
								t = t.." - Max icon amount of every row \n"
																
								t = t.."\124cff00FF00"
								t = t.."/eam SnameFontSize n"
								t = t.."\124r"
								t = t.." - Set SpellName font size for adjust and show NAME...\n"
								
								t = t.."\124cff00FF00"
								t = t.."/eam TimerFontSize n"
								t = t.."\124r"
								t = t.." - Set Timer font size for adjust and show TIMER...\n"
								
								t = t.."\124cff00FF00"
								t = t.."/eam StackFontSize n"
								t = t.."\124r"
								t = t.." - Set Stack font size for adjust and show STACK...\n"
								
								t = t.."\124cff00FF00"
								t = t.."/eam Play"
								t = t.."\124r"
								t = t.." - Play the Flash Animation, maybe you can put in Macro...\n"
								
								t = t.."\124cff00FF00"
								t = t.."/eam ExportClassToDef "
								t = t.."\124r"
								t = t.." - Export Class Setting Data to lua string for SpellArray.lua ...\n"
